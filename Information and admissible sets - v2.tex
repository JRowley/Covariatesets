...
I discuss the advantages of a single equation model, making specific reference to a complete structural model and to reduced form analysis. I discuss the credibility of Restriction \small{M}\normalsize{3} through Restriction \small{M}\normalsize{5}, theoretically and in the context of the application.  
...
To emphasise the advantage of a single equation model, I introduce two models. I state only the crucial restrictions that each model embeds, neglecting restrictions such as support restrictions that are of no relevance to the discussion. The first is described in \cite{hE78} and is an example of a more restrictive model (the multivariate probit model). The second is described in \cite{ai94} and is an example of a model that identifies a different feature (the local average treatment effect). I label these models \emph{H} and \emph{I}, respectively.
...
\end{enumerate}
A single equation model partially identifies a more valuable feature than reduced form analysis. The model that is described in \cite{ai94} (hereafter, the LATE model) embeds restrictions that impose weak constraints on assumed behaviour. Firstly, the LATE model restricts to be randomly assigned (Restriction A1). This restriction warrants more detailed discussion, especially since it marks a departure from the framework that I use. \cite{ai94} use a potential outcome framework whereas I follow \cite{p93} in that I use a causal effect framework. The two frameworks are shown to be equivalent \citep{p93}, but I suggest that the causal effect framework places more importance on establishing a map of causality.  
