...
\cite{bp97} assumes a non-parametric binary choice model. The model relaxes separability of unobservable heterogeneity in the structural equation for the outcome variable, but embeds a structural equation for the endogenous variable. That is, \cite{bp97} assumes a triangular model (whereas \cite{cr13} assumes a single equation model). The model is appealing in that it permits non-monotonic individual response and embeds only weak restrictions on the process that generates the endogenous variable. \cite{sHvY11} also assumes a triangular model and instead maintains separability of unobservable heterogeneity in the structural equation for the outcome variable, extending this to separability of unobservable heterogeneity in the structural equation for the outcome variable. Unlike \cite{bp97}, \cite{sHvY11} permits exogenous variables to enter the structural equation for the outcome variable and to enter the structural equation for the endogenous variable. 

\cite{c05} assumes a non-parametric binary choice model with continuous variation in the outcome variable. \cite{c05} assumes a triangular model that permits exogenous variables to enter the structural equation for the outcome variable and to enter the structural equation for the endogenous variable, although the way in which the structural equation for the outcome variable responds to variation in the exogenous variables is restricted. A monotonicity restriction means that the structural equation for the outcome variable can be bounded by observable quantiles of the conditional outcome distribution, but rules out binary choice since quantiles are uninformative in that case. \cite{kI09} 
