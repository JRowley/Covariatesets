...
The non-parametric binary choice model of \cite{cr13} is but one in a broad class of models that encompass the ideal of minimal restrictiveness and that partially identify the average causal effect of the endogenous variable on the outcome variable. I divide this class into non-parametric binary choice models, non-parametric choice models with continuous variation in the outcome variable, and ordered discrete choice models. I discuss some of these models and their merits relative to the non-parametric binary choice model of \cite{cr13}.

\cite{bp97} assumes a non-parametric binary choice model. The model relaxes separability of unobservable heterogeneity in the structural equation for the outcome variable, but embeds a structural equation for the endogenous variable. That is, \cite{bp97} assumes a triangular model (whereas \cite{cr13} assumes a single equation model). The model is appealing in that it permits non-monotonic individual response and embeds only weak restrictions on the process that generates the endogenous variable. \cite{sHvY11} also assumes a triangular model but maintains separability of unobservable heterogeneity in the structural equation for the outcome variable. \cite{sHvY11} restricts the structural equation for the endogenous variable to be separable in unobservable heterogeneity but permits exogenous variables to enter both structural equations.

\cite{c05} assumes a non-parametric binary choice model with continuous variation in the outcome variable. The model is triangular and permits exogenous variables to enter the structural equation for the outcome variable and to enter the structural equation for the endogenous variable.\footnote{\cite{c05} restricts the structural equation for the outcome variable to be locally invariant to variation in the exogenous variables. This is equivalent to an order condition.} The model embeds a monotonicity restriction and so the structural equation for the outcome variable can be bounded by observable quantiles of the conditional outcome distribution, but rules out binary choice since quantiles are uninformative in that case. \cite{kI09} extends \cite{bp97} by permitting continuous variation in the outcome variable. \cite{kI09} studies a variety of restrictions on covariation of the instrumental variables and unobservable heterogeneity, and compares the admissible set of values for the average causal effect of the endogenous variable on the outcome variable for each restriction. 

\cite{c10} assumes an ordered choice model. 

%
%\cite{c05} and \cite{kI09} permit continuous variation in the outcome variable whilst maintaining discrete variation in the endogenous variable, although \cite{c05} is unsuited to binary variation in the endogenous variable. \cite{c05} requires that a non-parametric triangular structural form be embedded in the model and that the structural equation for the outcome variable is monotonic in unobservable heterogeneity. \cite{c05} is unsuited to binary variation in the endogenous variable as the rank condition in this case is violated.  
%
