...
A relevant question is how to relate conditional causal effects to (unconditional) causal effects. More precisely, how does the average causal effect of the endogenous variable on the outcome variable relate to its conditional counterparts? I show that the average causal effect of the endogenous variable on the outcome variable can be expressed as a function of its conditional counterparts when the non-parametric binary choice model is augmented. I establish the conditions under which conditional causal effects can be informative about bounds on (unconditional) causal effects. That is, I establish the conditions under which bounds on conditional causal effects can be used to tighten bounds on (unconditional) causal effects, exploiting the mapping from one to the other.



Having studied the potential implications of incorporating information for a non-parametric binary choice model from a theoretical standpoint I consider application. As in \cite{cr13} I estimate the average causal effect of childbirth on a mother's labour force participation using \US census data. I extend \cite{cr13} by 
