...
I discuss the number of inequalities that describe the identified set, and how this number changes as $\dim(\mathcal{R}_X)$ increases. I show that there is a curse of dimensionality.
...
\[\dim(X)=K\text{ and }\dim(Z)=L,\]
and fix a particular ordering of the collection of $p(d,x)$ over $\mathcal{R}_D\times\mathcal{R}_X$. There are $2K$ parameters in the collection  of $p(d,x)$ over $\mathcal{R}_D\times\mathcal{R}_X$, and there are $2K\times L$ inequality relations for each parameter. Then, there are $2K!$ possible orderings of the collection of $p(d,x)$ over $\mathcal{R}_D\times\mathcal{R}_X$. In sum, the number of inequalities is 
\[2K!\times 4K^2\times L,\]
which grows in $K$ at a faster-than-exponential rate. 
\subsection{Computation} 
An algorithm for determining the inequalities governing the identified set.
\begin{enumerate}
