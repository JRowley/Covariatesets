...
I explore the effect of permitting $X$ to enter $h$ for the simple single equation model. That is, I explore the effect of assuming the simple single equation model versus assuming the single equation model. I show that $ACE(D\rightarrow Y)$ is a well-defined functional for the simple single equation model and for the single equation model. 

I note that $ACE(D\rightarrow Y)$ is defined as 
\[r(0)-r(1)\]
for the simple single equation model. Equivalently, I note that 
\[\mathbb{E}[Y(d)]=1-r(d)\]
for the simple single equation model. As such, $ACE(D\rightarrow Y)$ is a well-defined functional for the simple single equation model. Further, the relationship between $r$ and $p$ is 
\[r(d)=\sum_{x\in\mathcal{R}_X}p(d,x)\mathbb{P}(x),\]
although I stress that this relationship is purely theoretical and does not hold in practice.\footnote{In other words, estimation of $r$ in the simple single equation model is not equivalent to estimation of $p$ in the single equation model.} The relationship between $r$ and $p$ illustrates the 
\begin{itemize}
\item simple single equation model is appropriate if $X$ only matters in the aggregate (general equilibrium effects). 
\item need to show identification analysis for simple single equation model; sets $A$ and $B$ are not the same as the aggregation of the sets $A,B$ for the single equation model. 
\item Weighting function is conditional and so need not be equal; sets can shift therefore like point estimates when you include relevant $X$.
\item Context of application it is different.
\end{itemize}
%\begin{itemize}
...
\includegraphics[width=\textwidth]{C:/Users/Jeffro/Documents/GitHub/Covariatesets/Coding/tikzDevice/Instruments.pdf}
...
\includegraphics[width=\textwidth]{C:/Users/Jeffro/Documents/GitHub/Covariatesets/Coding/tikzDevice/Samesex.pdf}
\caption{}
\end{subfigure}
\caption{}
\label{fig:samesex}
...
\includegraphics[width=\textwidth]{C:/Users/Jeffro/Documents/GitHub/Covariatesets/Coding/tikzDevice/Multiple.pdf}
\caption{}
\end{subfigure}
\caption{}
\label{fig:multiple}
...
\includegraphics[width=\textwidth]{C:/Users/Jeffro/Documents/GitHub/Covariatesets/Coding/tikzDevice/Allin.pdf}
\caption{}
\end{subfigure}
\caption{}
\label{fig:allin}
