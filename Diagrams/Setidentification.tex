...
%  \draw [pilar] (r.north east) to [bend left=45] node [] {} (q.north west);
  ...
  \node (S) at (0.2,-1.2) [style={draw,shape=circle,fill=black,scale=0.2},label=right:$S_a$] {};
  \node (T) at (1.2,-1.5) [style={draw,shape=circle,fill=black,scale=0.2},label=right:$S_b$] {};
  \node (U) at (1,3) [style={draw,shape=circle,fill=black,scale=0.2},label=below:$S_c$] {};
  \draw [pil] (S.north west) to [bend right=15] node [] {} (P.north east);
  \draw [pil] (T.north west) to [bend right=45] node [] {} (P.east);
  \draw [pil] (U.north west) to [bend right=45] node [] {} (P.north);
%  \draw [pilar] (P.south east) to [bend right=30] node [] {} (S.south west);
%  \draw [pilar] (P.south) to [bend right=45] node [] {} (T.south west);
\end{tikzpicture}
\end{document}