...
# Simulate R draws from the 4-variate standard normal distribution.
R <- 10000000
...
Useful$X1 <- ifelse(Useful$boys2 == 1, 1, 0)
Useful$X2 <- ifelse(Useful$girls2 == 1, 1, 0)
Useful$X3 <- ifelse(Useful$boy1st == 1 & Useful$boy2nd == 0, 1, 0)
Useful$X4 <- ifelse(Useful$boy1st == 0 & Useful$boy2nd == 1, 1, 0)
# Regress Y on (X1,X2,X3) and compute estimated covariance matrix for the parameters.
Regression <- lm(Y ~ 0 + X1 + X2 + X3 + X4, data = Useful)
...
g$v3 <- Draws %*% g3
g$v4 <- Draws %*% g4
adjustment <- c(sqrt(sum(g1^2)), sqrt(sum(g2^2)), sqrt(sum(g3^2)), sqrt(sum(g4^2)))
